...
function getImageUrl(path) {
  ...
  // ✅ Case 3: Local asset (e.g. "/src/assets/red-roses.png")
  if (path.includes("/src/assets/")) {
    return new URL(path.replace("/src/", "../"), import.meta.url).href;
  }

  // ✅ Case 4: Plain filename (e.g. "red-roses.png")
...
onMounted(async () => {
  ...
  if (buyNowItem) {
    basket.value = [
      {
        product_id: {
          _id: buyNowItem.product_id, // ✅ Include actual product ID
          bouquet_name: buyNowItem.bouquet_name,
          price: buyNowItem.price,
          product_image: buyNowItem.image.replace("/src/assets/", ""),
        },
...
async function buyNow() {
  ...
  try {
    const formattedItems = basket.value.map((i) => ({
      product_id: i.product_id?._id, // ✅ Now correctly defined
      quantity: i.quantity,
    }));
