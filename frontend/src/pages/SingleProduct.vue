...
// 🖼️ Handles both local and uploaded product images
function getImageUrl(path) {
  if (!path) {
    return new URL("@/assets/placeholder.jpg", import.meta.url).href;
  }

  // ✅ Case 1: Backend uploaded image
  if (path.startsWith("/uploads/")) {
    return `http://localhost:3000${path}`;
  }

  // ✅ Case 2: Full link (CDN or http)
  if (path.startsWith("http")) {
    return path;
  }

  // ✅ Case 3: Local /src/assets/ file
  if (path.includes("/src/assets/")) {
    return new URL(path.replace("/src/", "../"), import.meta.url).href;
  }

  // ✅ Case 4: Plain file name (old products)
...
onMounted(async () => {
  ...
  price: found.price,
  image: getImageUrl(found.product_image),
...
async function goToCheckout() {
  ...
  // ✅ Go to checkout (will load this specific item)
