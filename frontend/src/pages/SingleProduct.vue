...
onMounted(async () => {
  ...
  description: found.description || "A beautiful flower arrangement perfect for any occasion.",
  includes: found.includes ? found.includes.split(";").map(i => i.trim()) : [],
};
...
async function goToCheckout() {
  ...
  try {
    ...
    console.error(err.response || err);
    alert(err.response?.data?.message || "❌ Failed to go to checkout.");
  }
}
